"""A real local moto server, for tests that read S3 through DuckDB.

DuckDB's httpfs extension makes its own socket connections, bypassing
the boto3/botocore interception ``moto.mock_aws`` relies on. Tests
that read S3 through DuckDB instead run against a real local moto
server. Plain ``boto3.client('s3')`` calls are pointed at it via
...
the ``endpoint`` test seam on ``configure`` and each handler, exposed
here as the ``_s3_endpoint`` fixture.
"""
