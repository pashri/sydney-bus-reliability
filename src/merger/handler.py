...
from src.compactor.handler import peak_rss_mb
...
def merge_trip_stops(
    *,
    ...
    connection.execute(
        f"COPY ({TRIP_STOP_MERGE}) TO '{target}' "
...
def handler(
    ...
    *,
    ...
    coverage = count_partial_coverage(bucket=bucket, window=window)
    ...
    CurationRepository(
        bucket=bucket, session=boto3.Session(),
